...
- **Corpus populated**: `spec/fixtures/accept/` and `reject/` filled with `{schema, input, expect}` cases (input = apply ops from an empty board); a `@whtbrd/core` test loads and runs every fixture and enforces enum closure. Log→projection fixtures remain A3.
...
- `packages/core/src/**` — new modules for wire types, tools, authoring kit, validation, plus their tests and the corpus runner; `zod` added as a runtime dependency of `@whtbrd/core` (the only new dependency).
...
- `@whtbrd/server` untouched except that it must still compile and pass against the new core exports.
