...
- [x] 6.2 Run the packet's verification: clean `pnpm check`; positive controls (break the `apply_ops` mapping → facade tests fail; let the unknown-board throw propagate → error-mapping test fails; flip one fixture expectation → MCP corpus runner fails; disable the WS poller → WS test fails; revert each). Show evidence, commit, push, output `<promise>A4-COMPLETE</promise>`.
  - Note on the unknown-board control: MCP SDK 1.30 already wraps a *thrown* handler into an `isError:true` result (`server/mcp.js` `createToolError`), so merely removing the facade's try/catch does **not** fail the error-mapping test — the SDK re-wraps. The facade's explicit catch is still the breakable seam the packet mandates; the faithful positive control makes `errorResult` drop `isError`, which fails both unknown-board tests. All four controls verified (apply_ops→20 fails; error-map→2 fails; fixture flip→1 fail; WS poller→4 fails) and reverted; cold `pnpm nx reset && pnpm check` green afterward.
