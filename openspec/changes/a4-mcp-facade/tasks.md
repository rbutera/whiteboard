...
- [x] 6.2 Run the packet's verification: clean `pnpm check`; positive controls (break the `apply_ops` mapping → facade tests fail; flip one fixture expectation → MCP corpus runner fails; disable the WS poller → WS test fails; revert each) — the unknown-board→`isError` contract is owned by the pinned SDK and pinned by the behavior test (see 6.2 note), not a facade catch to break. Show evidence, commit, push, output `<promise>A4-COMPLETE</promise>`.
  - Error-mapping contract (revised in A4 review): MCP SDK 1.30 wraps a *thrown* tool handler into an `isError:true` result (`server/mcp.js` `createToolError`). The facade therefore adds **no** try/catch mapping of its own — unknown-board `Error`s propagate and the pinned `@modelcontextprotocol/sdk ^1.30` does the wrapping. `facade.test.ts`'s unknown-board test pins the observable contract (isError:true + the real message) and fails if an SDK upgrade changes the wrapping. Cold `pnpm nx reset && pnpm check` green.
