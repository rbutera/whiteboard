...
- **Toolchain — boring on purpose**: `uv` (env + lock), `pydantic` v2 (wire models — #455 names Pydantic as the Python authoring surface), `pytest`, `mypy --strict`, `ruff` (format + lint). Compatible-release ranges in `pyproject.toml` (`pydantic>=2,<3`, `pytest>=8,<9`, `mypy>=1,<2`, exact-pinned `ruff`); the committed `uv.lock` is the exact pin. `.python-version` pins the dev/CI interpreter to 3.13; `requires-python = ">=3.12"` (PEP 695 typing floor, broad availability).
