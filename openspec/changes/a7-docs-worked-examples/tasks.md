...
- [x] 5.1 Fresh gate: `pnpm check` green with both example projects present in `pnpm nx show projects`; show the kanban and diagramming test tasks executing (cache-miss run acceptable via a real change earlier in the session — never `--skip-nx-cache` for cosmetics).
