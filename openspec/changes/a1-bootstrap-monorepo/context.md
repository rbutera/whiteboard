...
- Package skeletons `@whiteboard/core` and `@whiteboard/server` (server depends on core) — near-empty but genuinely building, typechecking, and passing at least one real test each, so the gate has a positive control.
...
- MIT `license` field in every package.json; `@whiteboard/*` scope; per-package semver (versions start `0.0.0`; nx release config is A5 — leave at most a note).
...
2. Positive control: deliberately break one assertion in a `@whiteboard/core` test and one type in `@whiteboard/server`; `pnpm check` must FAIL both times; revert.
3. Cross-package proof: a `@whiteboard/server` test imports from `@whiteboard/core` and passes.
