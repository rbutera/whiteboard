...
- **Versioning story (decided): fixed/locked, `0.1.0-alpha.N`, dist-tag `alpha`.**
  ...
  - *Dist-tag `alpha`*: an alpha must never be what a bare `npm install @wboard/core` resolves, and npm points `latest` at the first publish unless told otherwise. `"tag": "alpha"` goes in each package's `publishConfig` so even a bare publish cannot hit `latest`; the install proof pins `@alpha`. Graduation to `latest` is a later, deliberate release.
